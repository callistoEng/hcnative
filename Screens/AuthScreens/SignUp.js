...
const SignUpScreen = ({ navigation }) => {
  ...
  const confChange = (pwd) => {
    setData({ ...data, conf_password: pwd });
  };
  ...
  const conftogglePwdVissible = () => {
    setData({
      ...data,
      conf_secureTextEntry: !data.conf_secureTextEntry,
    });
  };
  return (
    <View style={styles.container}>
      ...
      <Animatable.View animation="fadeInUpBig" style={styles.footer}>
        ...
        <View style={styles.action}>
          ...
          <TextInput
            autoCapitalize="none"
            secureTextEntry={data.conf_secureTextEntry ? true : false}
            placeholder="confirn your password"
            style={styles.textInput}
            onChange={(pwd) => confChange(pwd)}
          />
          <TouchableOpacity onPress={conftogglePwdVissible}>
            {data.conf_secureTextEntry ? (
              <Feather color="grey" size={20} name="eye-off" />
            ) : (
              <Feather color="grey" size={20} name="eye" />
            )}
